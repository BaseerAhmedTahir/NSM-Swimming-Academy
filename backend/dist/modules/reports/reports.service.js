...
exports.createExcelReport = exports.createPdfReportStream = exports.generateReportData = void 0;
const database_1 = require("../../config/database");
const pdfkit_1 = __importDefault(require("pdfkit"));
const exceljs_1 = __importDefault(require("exceljs"));
/* Reports Service */
// Generates broad reporting data aggregated across modules.
const generateReportData = async (type, queryArgs) => {
    ...
    if (queryArgs.month && queryArgs.year) {
        ...
        startDate.setHours(0, 0, 0, 0);
        endDate = new Date(today.setDate(first + 7));
        endDate.setHours(23, 59, 59, 999);
    }
    ...
    if (type === 'attendance') {
        const records = await database_1.prisma.attendanceRecord.groupBy({
            by: ['status'],
    ...
    if (type === 'students') {
        const [statusBreakdown, levelBreakdown, total] = await Promise.all([
            database_1.prisma.student.groupBy({
                by: ['status'],
            ...
            database_1.prisma.student.count({ where: { ...whereBranch, createdAt: { gte: startDate, lte: endDate } } })
        ]);
...
const createPdfReportStream = async (type, data) => {
...
exports.createPdfReportStream = createPdfReportStream;
const createExcelReport = async (type, data) => {
